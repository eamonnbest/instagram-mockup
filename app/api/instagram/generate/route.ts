...
export async function POST(request: NextRequest) {
  try {
    ...
    if (referenceImageUrl) {
      ...
      const allowedImg2Img = ["fal-ai/flux-pro/kontext", "fal-ai/flux/krea/image-to-image", "fal-ai/flux-pro/v1.1-ultra"]
      ...
      if (img2imgModel === "fal-ai/flux-pro/v1.1-ultra") {
        input.raw = true
        input.aspect_ratio = "1:1"
        input.image_prompt_strength = strength
      } else if (img2imgModel === "fal-ai/flux/krea/image-to-image") {
      ...
      // Kontext doesn't support a strength param — it interprets the prompt contextually
    ...
    // Text-to-image mode
    const allowedModels = ["fal-ai/nano-banana-2", "fal-ai/gpt-image-1.5", "fal-ai/flux/krea", "fal-ai/flux-pro/v1.1-ultra", "fal-ai/bytedance/seedream/v4.5/text-to-image", "fal-ai/recraft/v3/text-to-image"]
    const selectedModel = allowedModels.includes(model) ? model : allowedModels[0]

    // FLUX Pro Ultra uses aspect_ratio + raw instead of image_size
